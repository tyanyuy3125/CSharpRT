...
sceneManager.AddObject(new Objects.Sphere(new Vec3(0, 0, 47), 44, new Emission(new Vec3(1,0,0),new Vec3(1,1,1))));
sceneManager.AddObject(new Objects.Sphere(new Vec3(10, 70, 20), 20, new Diffusion(new Vec3(1,1,1))));
sceneManager.AddObject(new Objects.Plane(new Vec3(0, 0, 1), new Vec3(0, 0, 0),new Diffusion(new Vec3(1,1,1))));
...
#region CornellBox
SceneManager CornellBox = new SceneManager();
...
CornellBox.AddObject(new Objects.Plane(new Vec3(0, 0, 1), new Vec3(0, 0, 0), new Diffusion(new Vec3(0.7, 0.7, 0.7))));
CornellBox.AddObject(new Objects.Plane(new Vec3(0, 1, 0), new Vec3(0, -400, 0), new Diffusion(new Vec3(0, 1, 0))));
CornellBox.AddObject(new Objects.Plane(new Vec3(0, -1, 0), new Vec3(0, 400, 0), new Diffusion(new Vec3(1, 0, 0))));
CornellBox.AddObject(new Objects.XYLimitedPlane(new Vec3(0, 0, -1), new Vec3(-65, 0, 700), 300, new Emission(new Vec3(1, 0.9, 0.6),10*new Vec3(1,0.9,0.7))));
CornellBox.AddObject(new Objects.Plane(new Vec3(0, 0, -1), new Vec3(0, 0, 700), new Diffusion(new Vec3(0.7, 0.7, 0.7))));

CornellBox.AddObject(new Objects.Plane(new Vec3(1, 0, 0), new Vec3(-250, 0, 0), new Diffusion(new Vec3(0.7, 0.7, 0.7))));
 ...
 *
 */
Vec2 res = new Vec2(800, 600);

Camera pathTracingCamera = new PathTracingCamera(new Vec3(150, 150, 44), new Vec3(-1, -1, 0), 10, new Vec2(36, 24));
Bitmap bitmap = new Bitmap((int)res.x, (int)res.y);

Visualizer visualizer = new Visualizer(new Size(res.xi, res.yi));

Task UIThread = new Task(() =>
{
    visualizer.ShowVisualizerWindow();
});
...
string Path = String.Format("D:\\RenderTest\\{0}.jpg", DateTime.Now.ToString("yyyyMMddHHmmss"));
if (!Directory.Exists("D:\\RenderTest\\"))
    Directory.CreateDirectory("D:\\RenderTest\\");
lock (bitmap)
{
